...
//#include <cylinder.h>
#define  NUM_ARRAY_ELEMENT(a) sizeof(a)/sizeof(*a);
...
{
	ShapeData ret;
	Vertex verts[] =   // put the triangle's point address into a array
	{
	...
	ret.numVertices = NUM_ARRAY_ELEMENT(verts);
	ret.vertices = new Vertex[ret.numVertices];
	memcpy(ret.vertices,verts,sizeof(verts));
	
	
	GLushort indices[] = {0,1,2};   // ushort stands for unsigned short
    ret.numIndices = NUM_ARRAY_ELEMENT(indices);
...
ShapeData ShapeGenerator::makeCube()
{
	...
	ret.numVertices = NUM_ARRAY_ELEMENT(stackVerts);	
	ret.vertices = new Vertex[ret.numVertices];
	...
	ret.numIndices = NUM_ARRAY_ELEMENT(stackIndices);
	ret.indices = new GLushort[ret.numIndices];
...
/*ShapeData ShapeGenerator::makeCylinder()
{
	ShapeData ret;
	Cylinder cy;
	const glm::vec4& topc = glm::vec4(1.0,1.0,0.0,1.0);
	const glm::vec4& botc  = glm::vec4(1.0,1.0,0.0,1.0);


	cy.generateVertices(0.9f,0.9f,0.9f,topc,botc,16);
	//Vertex stackVerts[] =cy.cyPositions;
